...
int16_t EcoMain(IEcoUnknown* pIUnk) {
    ...
    /* Получаем интерфейс формирования кадра */
    pIMessage = pICAN->pVTbl->get_Message(pICAN);

    /* Пример 1: Отправка всем кадра данных с маркировкой "67"
      ---------------                       -----------------
      |             |                       |               |
    ...
    */
    ...
    pIMessage->pVTbl->set_Identifier(pIMessage, 0x43);
    /* Устанавливаем длину сообщения, примечание: максимальная длина 8 байт */
    pIMessage->pVTbl->set_DLC(pIMessage, 8);
    /* Устанавливаем тип кадра "Data Frame" */
    pIMessage->pVTbl->set_RTR(pIMessage, 0);
    /* Устанавливаем сообщение */
    pIMessage->pVTbl->set_Data(pIMessage, msg2);

    /* Отправка сообщения */
    while (1) {
        result = pICAN->pVTbl->TransmittingMessage(pICAN, pIMessage);
        Delay(1000);
    }
